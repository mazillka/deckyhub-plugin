...
DeckyHub is a local-first plugin for Decky Loader. It tracks only GitHub repositories that you add or import, then lets you inspect and download matching release assets.
...
- Queries GitHub Releases directly from Steam's UI process. Release results are cached in `localStorage` for 15 minutes; a GitHub failure affects only that app card.
...
- `tests/` covers the Python backend.
...
Any user-managed repository can be removed. DeckyHub has no bundled repositories and does not scan installed official Decky plugins or add their repositories to Discover.
