...
type Settings = { verifySha256: boolean; overwriteExisting: boolean; updateChannel: UpdateChannel };
...
function SettingsPage() {
  const [settings, setSettings] = useState<Settings>({ verifySha256: true, overwriteExisting: false, updateChannel: "stable" });
...
function Content({ fullPage }: { fullPage?: View }) {
  ...
  const page = view === "updates" ? list((app) => app.updateAvailable === true, "No updates found") : view === "installed" ? list((app) => Boolean(app.installedVersion), "No supported apps installed") : list(() => true, "No repositories in registry");
