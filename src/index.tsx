...
export default definePlugin(() => { const fullscreen = (view: View) => <ScrollPanel><div style={{ paddingTop: 48 }}><Content fullPage={view} /></div></ScrollPanel>; routerHook.addRoute("/deckyhub/updates", () => fullscreen("updates")); routerHook.addRoute("/deckyhub/discover", () => fullscreen("discover")); routerHook.addRoute("/deckyhub/settings", () => <ScrollPanel><div style={{ paddingTop: 48 }}><SettingsPage /></div></ScrollPanel>); return { name: "DeckyHub", titleView: <div className={staticClasses.Title}>DeckyHub</div>, content: <Content />, icon: <FaGithub />, onDismount() { ["/deckyhub/updates", "/deckyhub/discover", "/deckyhub/settings"].forEach((path) => routerHook.removeRoute(path)); } }; });
